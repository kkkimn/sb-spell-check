...
with st.sidebar:
    st.header("⚙️ 설정")
    api_key_input = st.text_input("OpenAI API 키 (sk-...)", value=API_KEY_DEFAULT, type="password")
    
    tier_choice = st.radio(
        "API 요금제 선택",
        options=["무료 티어 (1초 딜레이 안전망)", "유료 티어 (딜레이 없는 최고 속도)"],
        index=1  # 기본값을 유료 요금제로 설정
    )
    is_paid_tier = "유료" in tier_choice
    
    st.info("""
    **🚨 최고 성능 OpenAI GPT-4o 모드**  
    가장 자연스러운 한국어 문맥 교정을 위해 OpenAI 엔진으로 구동됩니다. 발급받으신 `sk-` 로 시작하는 API 키를 입력해주세요.
    """)
    ...
    with col1:
        if st.button("🚀 AI 분석 및 텍스트 스캔 시작", use_container_width=True):
            ...
            if not api_key_input or not api_key_input.startswith("sk-"):
                st.error("올바른 OpenAI API 키를 왼쪽 설정창에 입력해 주세요! (sk-... 로 시작)")
                st.stop()
            ...
            def update_progress(current, total):
                ...
                status_text.markdown(f"**진행 상황:** {current}/{total} 장 스캔 완료... (무료 한도를 지키기 위해 대기하며 교정합니다)")
